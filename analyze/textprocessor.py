...
class TextProcessor(object):
    SHOT_RE = re.compile(
        "(MISS)?\s*([^']+|O'Quinn)\s+((\d+)')?\s*(((3PT|Tip|Alley Oop|Cutting|Dunk|Pullup|Pull-Up|Turnaround|Finger|Roll|Running|Driving|Hook|Putback|Step|Back|Jump|3pt|Layup|Floating|Fadeaway|Reverse|Bank|No) ?)*)\s*([Ss]hot|Layup|Jumper|Dunk|Fadeaway)\s*(\((\d*) PTS\))?\s*(\((.+) (\d+) AST\))?"
    )  # TODO: Track AST,BLK
    REBOUND_RE = re.compile(
        "(.+?) (Rebound|REBOUND)\s*(\(Off:\s*(\d+) Def:\s*(\d+)\))?"
    )
    DEFENSE_RE = re.compile("(.+?) (BLOCK|STEAL) \((\d+) (STL|BLK)\)")
    TIMEOUT_RE = re.compile(
        "([A-Za-z]+) ?Timeout: (Short|Regular|No Timeout|Official)(.*)"
    )
    ...
    VIOLATION_RE = re.compile(
        "(.+?) Violation:(Defensive Goaltending|Kicked Ball|Lane|Jump Ball|Double Lane)( )?"
    )
    FREE_THROW_RE = re.compile(
        "(MISS)?\s*(.+) Free Throw (Flagrant|Clear Path)? ?(\d) of (\d)\s*(\((\d+) PTS\))?"
    )
    ...
    EJECTION_RE = re.compile(
        "(.+?) Ejection:(First Flagrant Type 2|Second Technical|Other)"
    )
    ...
    def process_item(self, item):
        ...
        if text:
            ...
            if m:
                event = {
                    "type": eventTypes.JUMP,
                    "player_home": m.group(1),
                    "player_away": m.group(2),
                }
                if m.group(3):
                    pass
                    # event
            ...
            if m:
                event = {
                    "player": m.group(2),
                    "number": None,
                    "type": eventTypes.FREE_THROW,
                    "shot_type": "Free Throw",
                    "fta_no": int(m.group(4)),
                    "fta_ovr": int(m.group(5)),
                    "3pa": 0,
                    "shot_made": 0,
                    "ast_player": None,
                }
            ...
            if m:
                event = {
                    ...
                    "number": None,
                    "type": eventTypes.FREE_THROW,
                    "shot_type": "Free Throw Technical",
                    "fta_no": 1,
                    "fta_ovr": 1,
                    "3pa": 0,
                    "shot_made": 0,
