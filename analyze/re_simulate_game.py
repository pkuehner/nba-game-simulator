...
NUM_POSSESSIONS = 225  # Average for 2019



def sim_game(home_team: str, away_team: str):
    ...
    def calculate_percentage_for_possession(lineup_num, curr_team):
        ...
        if stats[curr_team][lineup_num]["poss"] != 0:
            for column in columns:
                percentages[column] += (
                    stats[curr_team][lineup_num][column]
                    / stats[curr_team][lineup_num]["poss"]
                )
        percentages["fta"] /= 2
        return percentages
        ...
        for index, column in enumerate(columns):
            if stats[curr_team][lineup_num][column] != 0:
                percentages[column[:-1]] += (
                    stats[curr_team][lineup_num][columns2[index]]
                    / stats[curr_team][lineup_num][column]
                )
        return percentages
    ...
    def change_lineup(lineup_num, curr_team):
        ...
        sub_percentages[curr_team] = calculate_percentage_for_substitution(
            lineup_num, curr_team
        )
        shot_percentages[curr_team] = calculate_shot_percentages(lineup_num, curr_team)
        poss_percentages[curr_team] = calculate_percentage_for_possession(
            lineup_num, curr_team
        )
    ...
    def change_possession(curr_team):
        if curr_team == "away":
            ...
            return "away"
    with open(f"processed_stats/{home_team}.json") as json_file:
        home_stats = json.load(json_file)

    with open(f"processed_stats/{away_team}.json") as json_file:
        away_stats = json.load(json_file)

    curr_team = "home"
    if np.random.rand() <= 0.5:
        curr_team = "away"
    home_stats = home_stats["home"]
    away_stats = away_stats["away"]
    ...
    lineups = {"away": away_lineups, "home": home_lineups}
    stats = {"away": away_stats_list, "home": home_stats_list}
    ...
    sub_percentages = {
        "home": calculate_percentage_for_substitution(0, "home"),
        "away": calculate_percentage_for_substitution(0, "away"),
    }
    poss_percentages = {
        "home": calculate_percentage_for_possession(0, "home"),
        "away": calculate_percentage_for_possession(0, "away"),
    }
    shot_percentages = {
        "home": calculate_shot_percentages(0, "home"),
        "away": calculate_shot_percentages(0, "away"),
    }
    reb_percentages = {
        "home": calculate_percentage_for_reb(0, "home"),
        "away": calculate_percentage_for_reb(0, "away"),
    }
    sub_percentage = 0.1
    scores = {"home": 0, "away": 0}
    quarter = 1
    while possessions != NUM_POSSESSIONS:
        possessions += 1
        x = np.random.rand()
        if x <= sub_percentage:
        ...
        if possessions == NUM_POSSESSIONS or (
            quarter < 4 and possessions == int(NUM_POSSESSIONS / 4) * quarter
        ):
            print("-----")
            print(f"Q{quarter} End", scores)
            print("-----")
            quarter += 1
...
def main():
    parser = argparse.ArgumentParser(description="Parse home and away team names.")
    ...
    sim_game(args.home_team, args.away_team)
